...
CREATE TABLE pending_submissions (
  id TEXT PRIMARY KEY,
  type TEXT NOT NULL,       -- 'feature_request' | 'feedback'
...
5. Feedback — link to feature_request_screen

---

### SCREEN 11: FEATURE REQUEST (`feature_request_screen.dart`)

**Entry points:**
- "Request a Feature" card on home screen
- Empty search result state
- Settings → Feedback

**Purpose:** Doctors submit two types of input:
1. Request a new calculator not yet in the app
2. General feedback on existing calculators or the app

**Layout (top to bottom):**
1. Back arrow + title "Request & Feedback"
2. Two tabs: "New Calculator" | "Feedback"

---

**Tab 1 — New Calculator Request:**

Fields:
- Calculator name / description (required) — large text input, placeholder: "e.g. Corrected Calcium, Osmolality, MEWS Score"
- Clinical use case (required) — text area, placeholder: "When do you use this? e.g. Managing hypercalcaemia patients in nephrology"
- Specialty (pre-filled from doctor profile, editable) — dropdown
- Priority — toggle: "Nice to have" | "I need this urgently"

Submit button: "Submit Request"

On submit:
- Write to Firestore `feature_requests` collection:
  ```
  {
    ...
    doctor_phone: string (anonymised — hashed),
    status: 'pending',
    votes: 1,
    created_at: timestamp
  }
  ```
- Show success state: "✓ Request submitted. We review all requests — thank you."
- Clear form
...
Fields:
- Type — dropdown: "Bug / Wrong result" | "Suggestion" | "Formula question" | "Other"
- Related calculator (optional) — dropdown populated from calculator registry
- Message (required) — text area, minimum 20 characters, placeholder: "Describe your feedback clearly..."
- Contact back? — toggle (default off). If on: shows phone number field pre-filled from profile
...
- Write to Firestore `feedback` collection:
  ```
  {
    ...
    message: string,
    contact: boolean,
    doctor_phone: string | null (only if contact = true),
    specialty: string,
    app_version: string,
    created_at: timestamp
  }
  ```
- Show success state: "✓ Feedback received. Thank you, Dr. [FirstName]."
- Clear form

---

**Offline behaviour:**
- If no internet: queue submission locally in SQLite table `pending_submissions`
- Sync to Firestore next time connectivity is available via CloudSyncService
- Show banner: "You're offline. Your submission will be sent when you reconnect."

**Pending submissions table (add to database schema):**
```sql
CREATE TABLE pending_submissions (
  id TEXT PRIMARY KEY,
  type TEXT NOT NULL,       -- 'feature_request' | 'feedback'
